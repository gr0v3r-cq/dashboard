			...
			(data) => {
				...
				for (var i = 0; i < data.length; i++) {
					if(JSON.parse(data[i].xprmser_data).prmser_idioma == 'es') {
						let cad_new = '{ "id": "'+ data[i].xprmser_id +'", "language": "'+ JSON.parse(data[i].xprmser_data).prmser_idioma +'", "description": "'+ JSON.parse(data[i].xprmser_data).prmser_descripcion +'", "urlimg": "'+ JSON.parse(data[i].xprmser_data).prmser_urlimg +'"}';
